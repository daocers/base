!function(c) {
	var b = function(e, d) {
		...
		this.values = c.grep(c.map(this.element.val().split(","), c.trim),
				function(f) {
					return f.length > 0
				});
		this.show()
	...
	b.prototype = {
		...
		show : function() {
			...
			d.element.wrap(c('<div class="tags">')).parent().on("click",
					function() {
						d.input.focus()
					});
			if (d.values.length) {
			...
			d.input = c('<input type="text">')
					.attr("placeholder", d.options.placeholder)
					.insertAfter(d.element)
					.on("focus", function() {
					...
					.on(
							...
							function() {
								if (!d.skip) {
									d.process();
									d.element.parent()
											.removeClass("tags-hover");
									d.element.siblings(".tag").removeClass(
											"tag-important")
								}
								d.skip = false
							})
					.on(
							"keydown",
							function(g) {
								if (g.keyCode == 188 || g.keyCode == 13
										|| g.keyCode == 9) {
									if (c.trim(c(this).val())
											&& (!d.element
													.siblings(".typeahead").length || d.element
													.siblings(".typeahead").is(
															":hidden"))) {
										if (g.keyCode != 9) {
											g.preventDefault()
										}
										...
										if (g.keyCode == 188) {
											if (!d.element
													.siblings(".typeahead").length
													|| d.element.siblings(
															".typeahead").is(
															":hidden")) {
												g.preventDefault()
											} else {
												d.input.data("typeahead")
														.select();
												g.stopPropagation();
												g.preventDefault()
											}
					...
					.typeahead(
							{
								...
								matcher : function(e) {
									return ~e.toLowerCase().indexOf(
											...
											&& (d.inValues(e) == -1 || d.options.allowDuplicates)
								},
		...
		process : function() {
			var d = c.grep(c.map(this.input.val().split(","), c.trim),
					function(f) {
						return f.length > 0
					}), e = this;
			c.each(d, function() {
